...
pub enum Node {
    Leaf(Id),
    Children(Id, Vec<Node>),
}

impl Node {
    pub fn name(node: &Node) -> Id {
        match node {
            Node::Leaf(id) => id.clone(),
            Node::Children(id, _) => id.clone(),
        }
    }
}


fn process_nodes(node_name: Id, orbit_map: &HashMap<Id, Vec<Id>>) -> Node {
    let children = orbit_map.get(&node_name).unwrap();

    if children.len() == 0 {
        return Node::Leaf(node_name);
    } else {
        let child_nodes =
            children.iter()
                    .map(|id| process_nodes(id.clone(), orbit_map))
                    .collect::<Vec<Node>>();

        return Node::Children(node_name, child_nodes);
    }
}

pub fn tree_size(node: &Node) -> usize {
    match node {
        Node::Leaf(id) => {
            return 1;
        }

        Node::Children(id, children) => {
            let mut sum = 1;
...
pub fn checksum(node: &Node) -> Checksum {
    match node {
        Node::Leaf(id) => {
            return 0;
        }
...
pub fn path_to_helper(target: Id, node: &Node, path: &mut Vec<Id>) -> bool {
    match node {
        Node::Leaf(id) => {
            return target == *id;
        }
        ...
        Node::Children(id, children) => {
            ...
            for child in children {
                ...
                found = path_to_helper(target.clone(), child, &mut new_path);
                if found {
                    path.clear();
...
pub fn path_to(target: Id, node: &Node) -> Vec<Id> {
    let mut path = Vec::new();

    path_to_helper(target, node, &mut path);
...
fn main() {
    let orbit_pairs =
     ...
     let mut orbit_map: HashMap<Id, Vec<Id>> = HashMap::new();

     for pair in orbit_pairs {
         if let Some(orbiters) = orbit_map.get_mut(&pair.0) {
             orbiters.push(pair.1.clone());
         } else {
             let orbiters = vec!(pair.1.clone());
             orbit_map.insert(pair.0, orbiters);
         }

         if orbit_map.get(&pair.1) == None {
             orbit_map.insert(pair.1, Vec::new());
         }
     }

     let orbit_tree = process_nodes("COM".to_string(), &orbit_map);

     // let orbit_checksum = checksum(&orbit_tree);
     let mut you_path = path_to("YOU".to_string(), &orbit_tree);
     you_path.reverse();

     let mut san_path = path_to("SAN".to_string(), &orbit_tree);
     san_path.reverse();

     let mut path_length = 0;
     for san_loc in san_path.iter() {
         path_length += 1;

         let mut you_path_length = 0;
         let mut found = false;
         for you_loc in you_path.iter() {
             you_path_length += 1;
             if san_loc == you_loc {
                 path_length += you_path_length;
                 found = true;
                 break;
             }
         }
