...
fn decode(code: &str) -> Command {
    use Direction::*;

    let dir = match code.chars().nth(0).unwrap() {
        'L' => Left,
        'R' => Right,
        'D' => Down,
        'U' => Up,
    ...
    let amount = code[1..].parse::<isize>().unwrap();
...
fn execute(commands: Vec<Command>) -> Path {
    use Direction::*;
    ...
    for command in commands {
        match command.dir {
            Left => {
                for ix in 1..=command.amount {
                    path.push((pos.0 - ix, pos.1));
                }
                pos = (pos.0 - command.amount, pos.1);
            }
            ...
            Up => {
                for ix in 1..=command.amount {
                    path.push((pos.0, pos.1 + ix));
                }
                pos = (pos.0, pos.1 + command.amount);
            }
...
fn path(path_string: &str) -> Path {
    let commands =
        path_string.split(",")
                   .map(|code| decode(code))
                   .collect::<Vec<Command>>();

    let path = execute(commands);

    return path;
}

fn overlaps(path1: Path, path2: Path) -> Path {
    let mut set1 = path1.iter().map(|val| *val).collect::<HashSet<Loc>>();
    let mut set2 = path2.iter().map(|val| *val).collect::<HashSet<Loc>>();

    set1.remove(&(0, 0));
    set2.remove(&(0, 0));

    let mut common = set1.intersection(&set2).map(|loc| *loc).collect::<Path>();

    return common;
}

fn closest_point_to_central(path: Path) -> isize {
    let min_dist = path.iter()
                       .map(|pos| (pos, pos.0.abs() + pos.1.abs()))
                       .min_by(|first, second| first.1.cmp(&second.1))
                       .unwrap();
...
fn part1() {
    ...
    let overlaps = overlaps(path1, path2);

    let overlap = closest_point_to_central(overlaps);

    println!("Result = {}", overlap);
}

fn shortest_distance(path1: Path, path2: Path) -> usize {
    let mut min_dist = 1000000000;

    for (index, loc) in path2.iter().enumerate() {
        if let Some(min_index) = path1.iter().position(|pos| pos == loc) {
            let dist = min_index + index;
            if dist != 0 {
